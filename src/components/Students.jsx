  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {list.map((s) => (
          <StudentCard key={s.id} s={s} onMessage={() => setMsgTo(s)} onEditJourney={() => setJourneyFor(s)} />
        ))}
      </div>
...
function StudentCard({ s, onMessage, onEditJourney }) {
  ...
  return (
    <div className="bg-white border border-slate-100 rounded-3xl p-5 shadow-sm hover:shadow-md transition-shadow">
      ...
      <button
        ...
        className="mt-3 w-full flex items-center justify-center gap-2 rounded-full border border-orange-200 text-orange-600 hover:bg-orange-50 transition-colors text-sm font-bold py-2.5"
      >
